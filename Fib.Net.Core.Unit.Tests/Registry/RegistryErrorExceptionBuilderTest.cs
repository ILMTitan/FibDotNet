...
namespace Fib.Net.Core.Unit.Tests.Registry
{
    ...
    public class RegistryErrorExceptionBuilderTest
    {
        ...
        public void TestAddErrorEntry()
        {
            ...
            {
                Assert.AreEqual(
                    "Tried to do something but failed because: manifest invalid (something went wrong), blob unknown (something went wrong), manifest unknown, tag invalid, manifest unverified, other: some other error happened, unknown: some unknown error happened | If this is a bug, please file an issue at https://github.com/GoogleContainerTools/fib/issues/new",
                    ex.Message);
            }
        }
    }
